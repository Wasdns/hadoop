...
blackbox_type meter {
    ...
    */
    ...
    method execute_with_or (out bit<0> destination, optional in int index){}

    /* Same as execute, but index is derived from a field list calculation. */
    method execute_from_hash(out bit<0> destination, in field_list_calculation hash_field_list){}

    /* Same as execute_from_hash, but destination field is OR'd with meter result. */
    method execute_from_hash_with_or(out bit<0> destination, in field_list_calculation hash_field_list){}
    ...
    method execute_with_pre_color_with_or(out bit<0> destination, in bit<0> precolor, optional in int index){}

    /* Same as execute_with_pre_color, but index is derived from a field list calculation. */
    method execute_with_pre_color_from_hash(out bit<0> destination, in bit<0> precolor, in field_list_calculation hash_field_list){}

    /* Same as execute_with_pre_color_from_hash, but destination field is OR'd with meter result. */
    method execute_with_pre_color_from_hash_with_or(out bit<0> destination, in bit<0> precolor, in field_list_calculation hash_field_list){}
}
